...
public class FileContentUtil {
    ...
    /**
     * 搜索文件内容(忽略大小写) 并打印文件绝对路径
     * @param file
     * @param searchStr
     */
    public void searchContentIgnoreCase(File file, String searchStr){
        if (file.isFile()) {
            try(InputStream inputStream = new FileInputStream(file);
                InputStreamReader inputStreamReader = new InputStreamReader(inputStream);
                BufferedReader bufferedReader = new BufferedReader(inputStreamReader);){
                ...
                if(stringBuilder.toString().toLowerCase().contains(searchStr.toLowerCase())){
                    System.out.println(file.getAbsolutePath());
                }
            }catch (Exception e){
                e.printStackTrace();
            ...
            File[] files = file.listFiles();
            for (File file2 : files) {
                searchContentIgnoreCase(file2, searchStr);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        //查找文件内容
        //  new FileContentUtil()
        // .searchContent(new File("D:\\kettle_file\\售后（Oracle）"),
        //  "plan_table");

        //查找文件内容忽略大小写
         new FileContentUtil()
        .searchContentIgnoreCase(new File("D:\\kettle_file\\售后（Oracle）"),
         "bin_");
    }
